...
Scheduled story generation is handled by the Supabase/Vercel story pipeline. GitHub Actions should not generate or commit stories. Social posting can be triggered manually through `social-media-dispatch.yml` or run locally with `scripts/social-media-poster.js` when the relevant platform tokens are configured.
