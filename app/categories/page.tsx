  ...
  return (
    <div className="bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Category Tree - Showing main categories with their subcategories */}
        <div className="mb-16">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Category Tree</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {mainCategories.filter(cat => getSubcategories(cat.slug).length > 0).map((mainCat) => {
              const subcats = getSubcategories(mainCat.slug);
              return (
                <div key={mainCat.slug} className="bg-white rounded-lg shadow-sm p-6 border border-gray-100">
                  <Link
                    href={`/categories/${mainCat.slug}`}
                    className="flex items-center gap-3 mb-4 hover:text-[#C9A14A] transition-colors"
                  >
                    <span className="text-3xl">{mainCat.icon}</span>
                    <h3 className="text-xl font-semibold">{mainCat.name}</h3>
                  </Link>
                    ...
                    {subcats.map(subcat => (
                      <li key={subcat.slug}>
                        <Link
                          ...
                          className="text-gray-700 hover:text-[#C9A14A] transition-colors flex items-center gap-2"
                        >
