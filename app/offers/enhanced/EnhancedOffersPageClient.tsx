  ...
  return (
    <main className="min-h-screen bg-[#f7f5ef] text-[#172536]">
      ...
      <section id="partner-directory" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="space-y-16">{visibleCategories.map(([category, partners]) => { const Icon = getCategoryIcon(category); return <section key={category}><div className="mb-6 flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#eee8d8]"><Icon className="h-5 w-5 text-[#8a6a20]" /></div><div><h3 className="text-2xl font-black">{CATEGORY_LABELS[category] || category}</h3><p className="text-sm text-[#71808e]">{partners.length} trusted option{partners.length === 1 ? '' : 's'}</p></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{partners.map(partner => <article key={partner.name} className="group flex h-full flex-col rounded-2xl border border-[#ded8cb] bg-white p-6 transition hover:-translate-y-1 hover:border-[#c9a14a] hover:shadow-lg"><div className="flex items-center justify-between gap-4"><div className="relative h-10 w-28"><Image src={partner.logo} alt={`${partner.name} logo`} fill className="object-contain object-left" sizes="112px" /></div><span className="text-xs font-bold uppercase tracking-wider text-[#9a8f7e]">Partner</span></div><h4 className="mt-7 text-xl font-black">{partner.name}</h4><p className="mt-2 flex-1 text-sm leading-6 text-[#536170]">{partner.description}</p><button type="button" onClick={() => handleDealClick(partner.name, partner.url)} className="mt-6 inline-flex items-center gap-2 self-start border-b-2 border-[#c9a14a] pb-1 text-sm font-black text-[#172536] transition group-hover:text-[#8a6a20]">Visit {partner.name} <ArrowUpRight className="h-4 w-4" /></button></article>)}</div></section>; })}</div>
      </section>
